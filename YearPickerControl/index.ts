...
export class YearPickerControl implements ComponentFramework.StandardControl<IInputs, IOutputs> {
    ...
    private onSelectYear(year: number): void {
        // Ensure that we're updating the context in a manner consistent with PCF framework guidelines
        const newDate = new Date(year, 0, 1);
        if (this.context.parameters.dateInput.raw) {
            this.context.parameters.dateInput.raw = newDate;
...
const YearPicker: React.FunctionComponent<IYearPickerProps> = (props) => {
    const selectedDate: Date = props.selectedYear ? new Date(parseInt(props.selectedYear, 10), 0, 1) : new Date();

    const onSelectDate = (date: Date | null | undefined) => {
        if (date) {
            props.onSelectYear(date.getFullYear());
        }
    };
